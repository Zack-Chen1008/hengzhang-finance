...
type User = { id:string; email:string; name:string; role:Role; status:string; mustChangePassword:boolean; departmentId:string|null; createdAt:string };
...
export default function FinanceApp() {
  ...
  const canApprove = (transaction:Transaction) => Boolean(current && (current.role === 'super_admin' || (stageRoles[transaction.status] === current.role && (current.role!=='manager'||Boolean(current.departmentId&&current.departmentId===transaction.departmentId)))));
  ...
  const totals = useMemo(() => {
    ...
    const openingBalance=data?.organizationItems.filter((item)=>['bank','cash'].includes(item.kind)).reduce((sum,item)=>sum+item.openingBalance,0)??0;
    return {openingBalance,income,expense,balance:openingBalance+income-expense,pending:pending.reduce((sum,item) => sum + item.amount,0)};
  },[data,pending]);
  ...
  async function submitTransaction(event:FormEvent<HTMLFormElement>) {
    ...
    const body = { id:existing?.id, type:String(form.get('type')), subject:String(form.get('subject')), counterparty:String(form.get('counterparty')), amount:Number(form.get('amount')), note:String(form.get('note')), accountId:String(form.get('accountId')), departmentId:String(form.get('departmentId')), projectId:String(form.get('projectId')), categoryId:String(form.get('categoryId')), createdAt:existing?.createdAt, attachmentCount:existing?.attachmentCount };
    setBusy(true);
  ...
  async function submitUser(event:FormEvent<HTMLFormElement>) {
    ...
    const body={id:existing?.id,email:String(form.get('email')),name:String(form.get('name')),role:String(form.get('role')),status:String(form.get('status')??'active'),departmentId:String(form.get('departmentId')??'')}; setBusy(true);
  ...
  function exportExcel(rows=filteredTransactions) {
    const itemName=(id:string|null)=>appData.organizationItems.find((entry)=>entry.id===id)?.name??'—';
    const headers=['单据编号','类型','业务事项','往来单位/人员','账户','部门','项目','收支分类','金额','状态','说明','创建时间'];
    const bodyRows=rows.map((item)=>[item.id,item.type==='income'?'收款':'付款',item.subject,item.counterparty,itemName(item.accountId),itemName(item.departmentId),itemName(item.projectId),itemName(item.categoryId),item.amount,item.status,item.note,dateText(item.createdAt)]);
  ...
  function renderDashboard(){
    const metrics=[['本期收入',money(totals.income),'income'],['本期支出',money(totals.expense),'expense'],['账面结余',money(totals.balance),'balance'],['期初余额',money(totals.openingBalance),'pending']];
  ...
  function renderTransactions(){
    ...
    const itemName=(id:string|null)=>appData.organizationItems.find((entry)=>entry.id===id)?.name??'—';
    return <section className="panel data-panel"><div className="filters"><input aria-label="搜索账务" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="搜索单号、事项或往来单位"/><select value={typeFilter} onChange={(e)=>setTypeFilter(e.target.value)}><option value="all">全部类型</option><option value="income">收款</option><option value="expense">付款</option></select><select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)}><option value="all">全部状态</option>{statuses.map((status)=><option key={status}>{status}</option>)}</select><span>共 {filteredTransactions.length} 笔</span></div><div className="table-wrap"><table><thead><tr><th>单据编号</th><th>业务事项</th><th>账户 / 分类</th><th>部门 / 项目</th><th>金额</th><th>状态</th><th>附件</th><th>操作</th></tr></thead><tbody>{filteredTransactions.map((item)=><tr key={item.id}><td>{item.id}</td><td><strong>{item.subject}</strong><small>{item.counterparty} · {dateText(item.createdAt)}</small></td><td><strong>{itemName(item.accountId)}</strong><small>{itemName(item.categoryId)}</small></td><td><strong>{itemName(item.departmentId)}</strong><small>{itemName(item.projectId)}</small></td><td className={item.type==='income'?'amount-in':'amount-out'}>{item.type==='income'?'+ ':'- '}{money(item.amount)}</td><td><Status value={item.status}/></td><td>{item.attachmentCount} 个</td><td><div className="row-actions"><button onClick={()=>setModal({type:'detail',item})}>详情</button>{['待部门审批','待财务确认','已驳回'].includes(item.status)?<button onClick={()=>{setTransactionFormType(item.type);setModal({type:'transaction',item})}}>修改</button>:null}{isAdmin?<button className="danger" onClick={()=>void removeTransaction(item)}>删除</button>:null}</div></td></tr>)}</tbody></table>{filteredTransactions.length===0?<Empty title="没有符合条件的单据" text="调整筛选条件或新建一笔收付款"/>:null}</div></section>;
  }
  ...
  function renderReports(){
    ...
    return <><section className="report-hero"><div><p>含期初余额的账面口径</p><strong>{money(totals.openingBalance+income-expense)}</strong><span>当前账面余额</span></div><div className="report-split"><span>期初余额<strong>{money(totals.openingBalance)}</strong></span><span>已确认收入<strong>{money(income)}</strong></span><span>已完成支出<strong>{money(expense)}</strong></span><span>完成单据<strong>{completed.length} 笔</strong></span></div><button onClick={()=>exportExcel(appData.transactions)}>导出全部 Excel</button></section><section className="content-grid"><article className="panel"><div className="panel-title"><div><h2>业务类型分布</h2><p>按全部有效单据计算</p></div></div><div className="distribution"><div style={{'--value':`${totals.income+totals.expense?totals.income/(totals.income+totals.expense)*100:0}%`} as CSSProperties}><span>{totals.income+totals.expense?Math.round(totals.income/(totals.income+totals.expense)*100):0}%</span></div><dl><dt><i className="income-dot"/>收入</dt><dd>{money(totals.income)}</dd><dt><i className="expense-dot"/>支出</dt><dd>{money(totals.expense)}</dd></dl></div></article><article className="panel"><div className="panel-title"><div><h2>流程完成情况</h2><p>按单据当前状态统计</p></div></div><div className="progress-list"><Progress label="已完成" count={appData.transactions.filter(i=>i.status==='已完成').length}/><Progress label="审批中" count={pending.length}/><Progress label="已驳回" count={appData.transactions.filter(i=>i.status==='已驳回').length}/></div></article></section></>;
  }

  function renderModal(){
    if(!modal)return null;
    if(modal.type==='transaction'){const item=modal.item;const accounts=appData.organizationItems.filter((entry)=>['bank','cash'].includes(entry.kind));const allDepartments=appData.organizationItems.filter((entry)=>entry.kind==='department');const departments=['employee','manager'].includes(appUser.role)?allDepartments.filter((entry)=>entry.id===appUser.departmentId):allDepartments;const projects=appData.organizationItems.filter((entry)=>entry.kind==='project');const categories=appData.organizationItems.filter((entry)=>entry.kind===(transactionFormType==='income'?'income_category':'expense_category'));return <Modal title={item?'修改单据':'新建收付款单据'} subtitle="账户、部门与分类将进入统计和权限控制"><form onSubmit={submitTransaction}><div className="field-row"><label>业务类型<select name="type" value={transactionFormType} onChange={(event)=>setTransactionFormType(event.target.value as 'income'|'expense')}><option value="expense">付款 / 报销</option><option value="income">收款</option></select></label><label>金额（元）<input name="amount" type="number" min="0.01" step="0.01" defaultValue={item?.amount} required placeholder="0.00"/></label></div><label>业务事项<input name="subject" defaultValue={item?.subject} maxLength={80} required placeholder="例如：办公设备采购"/></label><label>往来单位 / 人员<input name="counterparty" defaultValue={item?.counterparty} maxLength={80} required placeholder="请输入客户、供应商或员工姓名"/></label><div className="field-row"><label>资金账户<select name="accountId" defaultValue={item?.accountId??accounts[0]?.id??''} required><option value="" disabled>请选择账户</option>{accounts.map((entry)=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label><label>部门<select name="departmentId" defaultValue={item?.departmentId??appUser.departmentId??departments[0]?.id??''} required><option value="" disabled>请选择部门</option>{departments.map((entry)=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label></div><div className="field-row"><label>项目（可选）<select name="projectId" defaultValue={item?.projectId??''}><option value="">不关联项目</option>{projects.map((entry)=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label><label>{transactionFormType==='income'?'收入分类':'支出分类'}<select name="categoryId" key={transactionFormType} defaultValue={(item?.type===transactionFormType?item.categoryId:null)??categories[0]?.id??''} required><option value="" disabled>请选择分类</option>{categories.map((entry)=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label></div><label>说明<textarea name="note" rows={3} maxLength={500} defaultValue={item?.note} placeholder="合同、用途等补充信息"/></label><div className="form-tip">付款与报销：本部门负责人 → 财务 → 老板 → 出纳；收款：财务确认</div><FormActions/></form></Modal>}
    ...
    if(modal.type==='user'){const item=modal.item;const roleOptions=item?.role==='super_admin'?[...assignableRoles,'super_admin' as Role]:assignableRoles;const departments=appData.organizationItems.filter((entry)=>entry.kind==='department');return <Modal title={item?'修改人员权限':'添加员工'} subtitle="员工使用这里填写的邮箱登录"><form onSubmit={submitUser}><label>姓名<input name="name" defaultValue={item?.name} maxLength={50} required/></label><label>登录邮箱<input name="email" type="email" defaultValue={item?.email} disabled={Boolean(item)} required/></label><div className="field-row"><label>岗位<select name="role" defaultValue={item?.role??'employee'}>{roleOptions.map((role)=><option key={role} value={role}>{roleNames[role]}</option>)}</select></label><label>所属部门<select name="departmentId" defaultValue={item?.departmentId??departments[0]?.id??''}><option value="">全公司 / 不限定部门</option>{departments.map((entry)=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label></div>{item?<label>账号状态<select name="status" defaultValue={item.status}><option value="active">正常</option><option value="disabled">停用</option></select></label>:null}<div className="form-tip">普通员工与部门负责人必须选择部门；负责人只能查看和审批本部门单据。新员工初始密码为 123456。</div><FormActions/></form></Modal>}
