...
import { addNotification, errorResponse, makeId, notifyRole, requireAppUser, Role, writeAudit } from '../../lib/server';
...
export async function POST(request:Request) {
  try {
    ...
    const transaction = await env.DB.prepare('SELECT status,subject,created_by FROM transactions WHERE id = ?').bind(transactionId).first<Record<string, unknown>>();
    ...
    if(action==='reject')await addNotification(String(transaction.created_by),'单据已驳回',`${String(transaction.subject)}：${comment||'请修改后重新提交'}`,'workflow');
      ...
      const nextRole=stages[nextStatus]?.role;if(nextRole)await notifyRole(nextRole,'新的审批任务',`${String(transaction.subject)} · 当前环节：${nextStatus}`);
