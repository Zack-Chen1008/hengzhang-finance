...
import { errorResponse, makeId, requireAppUser, requireRole, Role } from '../../lib/server';
...
export async function POST(request:Request) {
  try {
    ...
    if (!/^\S+@\S+\.\S+$/.test(email) || !name || name.length > 50 || !roles.includes(role)) return Response.json({ error:'请填写正确的姓名、邮箱和岗位' }, { status:400 });
    const id = makeId('USR');
    const now = new Date().toISOString();
    try {
      await env.DB.prepare('INSERT INTO app_users (id,auth_user_id,email,name,role,status,created_at,updated_at) VALUES (?,NULL,?,?,?,?,?,?)')
        .bind(id,email,name,role,'active',now,now).run();
    } catch {
      return Response.json({ error:'该邮箱已经存在' }, { status:409 });
    }
    return Response.json({ item:{ id,email,name,role,status:'active',createdAt:now } }, { status:201 });
