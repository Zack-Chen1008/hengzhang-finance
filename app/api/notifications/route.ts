...
export async function POST(request:Request){
  try{const user=await requireAppUser();requireRole(user,['super_admin']);const body=await request.json() as Record<string,unknown>;const action=String(body.action??'');
    ...
    if(action==='test'){await addNotification(user.id,'通知测试','站内消息渠道工作正常。','test');await sendExternalNotification('衡账通知测试','外部通知渠道配置成功。','test');return Response.json({message:'测试消息已发送并记录结果'})}
