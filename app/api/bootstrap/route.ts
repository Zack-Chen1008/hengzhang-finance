...
import { maybeCreateAutomaticBackup } from '../../lib/backup';
import { addNotification, errorResponse, mapTransaction, requireAppUser } from '../../lib/server';

export async function GET() {
  try {
    const currentUser = await requireAppUser();
    if(currentUser.role==='super_admin')await maybeCreateAutomaticBackup(currentUser);
    if(['super_admin','finance','owner','cashier'].includes(currentUser.role)){
      const due=await env.DB.prepare("SELECT id,subject,counterparty,due_date FROM payment_plans WHERE status='pending' AND due_date<=date('now','+3 day') ORDER BY due_date LIMIT 50").all<Record<string,unknown>>();
      const today=new Date().toISOString().slice(0,10);for(const plan of due.results)await addNotification(currentUser.id,'收付款即将到期',`${String(plan.subject)} · ${String(plan.counterparty)} · ${String(plan.due_date)}`,'due',`due:${currentUser.id}:${String(plan.id)}:${today}`);
    }

    const employeeOnly=currentUser.role==='employee';
    const transactionStatement=employeeOnly
      ? env.DB.prepare(`SELECT t.*, (SELECT COUNT(*) FROM attachments a WHERE a.transaction_id=t.id) AS attachment_count FROM transactions t WHERE t.created_by=? ORDER BY t.created_at DESC LIMIT 500`).bind(currentUser.id)
      : env.DB.prepare(`SELECT t.*, (SELECT COUNT(*) FROM attachments a WHERE a.transaction_id=t.id) AS attachment_count FROM transactions t ORDER BY t.created_at DESC LIMIT 500`);
    const [company,users,partners,transactions,approvals,attachments,resetRequests,organizationItems,invoices,plans,bankRows,notifications,notificationSettings,auditLogs,deletedRecords,backups] = await Promise.all([
      env.DB.prepare('SELECT company_name FROM company_settings WHERE id=1').first<Record<string,unknown>>(),
      currentUser.role==='super_admin'?env.DB.prepare('SELECT id,email,name,role,status,must_change_password,created_at FROM app_users ORDER BY created_at').all():Promise.resolve({results:[]}),
      ...
      employeeOnly?env.DB.prepare('SELECT a.* FROM approvals a JOIN transactions t ON t.id=a.transaction_id WHERE t.created_by=? ORDER BY a.created_at DESC LIMIT 1000').bind(currentUser.id).all():env.DB.prepare('SELECT * FROM approvals ORDER BY created_at DESC LIMIT 1000').all(),
      employeeOnly?env.DB.prepare('SELECT a.id,a.transaction_id,a.filename,a.content_type,a.size,a.created_at FROM attachments a JOIN transactions t ON t.id=a.transaction_id WHERE t.created_by=? ORDER BY a.created_at DESC LIMIT 1000').bind(currentUser.id).all():env.DB.prepare('SELECT id,transaction_id,filename,content_type,size,created_at FROM attachments ORDER BY created_at DESC LIMIT 1000').all(),
      ...
      ['super_admin','finance','owner','cashier'].includes(currentUser.role)?env.DB.prepare("SELECT * FROM organization_items WHERE status='active' ORDER BY kind,name").all():Promise.resolve({results:[]}),
